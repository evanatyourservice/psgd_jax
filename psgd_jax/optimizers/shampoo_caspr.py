...
from psgd_jax.optimizers.quantization_utils import QuantizedValue
...
def matrix_inverse_pth_root(
    ...
    if lobpcg_topk_precondition > 0:
        ...
        lobpcg_diagnostics = LOBPCGDiagnostics.create(
            matrix, eigvals, eigvecs, lobpcg_iters
        )
    ...
    max_error_ratio = 1.2  # TODO (evanatyourservice): not used

    def _iter_condition(state):
        i, unused_mat_m, unused_mat_h, unused_old_mat_h, error, error_ratio = state
        error_above_threshold = error > error_tolerance
    ...
    if matrix_size == 1:
        ...
        total_retries, resultant_mat_h, error, iters, error_ratio, _ = (
            jax.lax.while_loop(
...
class Preconditioner:
    ...
    def updated_statistics_from_grad(
        self, stats, grad, w1, w2, to_float=None, from_float=None, precision=None
    ):
...
def distributed_shampoo(
    ...
    preconditioning_compute_steps=32,
    ...
    precision=lax.Precision.HIGHEST,
    tensordot_precision=lax.Precision.HIGH,
    relative_matrix_epsilon=True,
    ...
    start_preconditioning_step = max(
        start_preconditioning_step, preconditioning_compute_steps
    )
    ...
    def sharded_update_fn(grads, state, params):
        ...
        for local_stat in local_stats_flat:
            stats_flat.append(_convert_to_parameter_stats(global_stats, local_stat))
    ...
    def _quantized_matrix_inverse_pth_root_vmap(
        qxs, qds, qbs, ps, padding_starts, qpxs=None, qpds=None, qpbs=None
    ):
        ...
        def matrix_inverse_pth_root_wrapper(
            qx, qd, qb, p, padding_start, qpx, qpd, qpb
        ):
        ...
        return jax.vmap(matrix_inverse_pth_root_wrapper)(
            qxs, qds, qbs, ps, padding_starts, qpxs, qpds, qpbs
        )
    ...
    def _pmap_quantized_compute_preconditioners(
        ...
        if reuse_preconditioner:
            ...
            packed_quantized_precond_mats = pad_and_maybe_zero_preconditioners(
                [p.quantized for p in prev_preconditioners], total, max_size, step
            )
        ...
        all_exponents = batch(exponents, num_devices)
        all_paddings = batch(paddings, num_devices)
        all_quantized_precond_mats = _maybe(batch)(
            packed_quantized_precond_mats, num_devices
        )
